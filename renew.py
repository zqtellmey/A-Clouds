        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    debug_msg = f"<b>Groq 识别调试 (第 {idx_group + 1} 组)</b>\n目标: <code>{target_word}</code>\n<pre>{raw_text}</pre>"
                    send_tg_msg(debug_msg)
    ...
    if await prompt_locator.count() > 0:
        ...
        if count == 4:
            ...
            for i in range(4):
                ...
                raw_bytes = await img_element.screenshot(type="jpeg", quality=80)
                # 校验并自适应调整图片尺寸以满足 Groq 最低 32px 限制
                safe_bytes = ensure_min_image_size(raw_bytes, min_size=32)
                image_bytes_list.append(safe_bytes)
